...
description: Attach OpenCode browser tools to the Mitra (OpenWork) Electron app during local development and drive its UI over Chrome DevTools Protocol. Use only when developing Mitra itself (pnpm dev) to smoke-test the composer and confirm a session response — not for end-user content tasks.
---
...
```sh
nohup pnpm dev > /var/folders/d9/xqhkvsp94rg0n0n523snqztm0000gn/T/opencode/mitra-dev.log 2>&1 &
```
