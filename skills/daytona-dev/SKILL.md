---
name: daytona-dev
description: Launch the full Mitra (OpenWork) stack — Electron app plus Den — in a Daytona cloud sandbox on a virtual display, reachable in a browser via noVNC. Use only when developing Mitra itself in a remote sandbox — not for end-user content tasks.
---
...
```bash
daytona exec mitra-dev 'bash .devcontainer/start-services.sh'
```

Or SSH in and run interactively:

```bash
daytona ssh mitra-dev
cd /workspace
...
```bash
daytona preview-url mitra-dev -p 6080
```
...
daytona preview-url mitra-dev -p 3005

# CDP debugging endpoint
daytona preview-url mitra-dev -p 9825
```
...
# Check logs
daytona exec mitra-dev 'tail -50 /tmp/electron.log'
daytona exec mitra-dev 'tail -50 /tmp/vite.log'

# Take a screenshot via CDP
daytona exec mitra-dev 'curl -s http://127.0.0.1:9825/json/list'

# Restart just the Electron app
daytona exec mitra-dev 'pkill -f electron; sleep 2; cd /workspace && DISPLAY=:99 ELECTRON_DISABLE_SANDBOX=1 OPENWORK_ELECTRON_REMOTE_DEBUG_PORT=9825 OPENWORK_DEV_MODE=1 nohup pnpm --filter @mitra/desktop dev:electron > /tmp/electron.log 2>&1 &'

# Stop the sandbox (preserves state)
daytona stop mitra-dev

# Start it again
daytona start mitra-dev

# Delete (destroys everything)
daytona delete mitra-dev
```
